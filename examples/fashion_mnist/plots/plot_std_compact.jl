...
ax = Axis(fig[1,1], xlabel="Resolution", ylabel="Spatial standard deviation", title="Baseline Net", xticks=xticks, titlealign=:left)
violin!(ax, x, sqrt.(y), side = side, color = color, label="data")
...
p = violin!(ax, x, sqrt.(y), side = side, color = color, label=["data", "generated"])
xlims!(ax, min_x, max_x)
ylims!(ax, min_y, max_y)
...
if smooth
    save("fig_std_compact_epochs$(epochs)_smooth.png", fig, px_per_unit = 2)
else
    save("fig_std_compact_epochs$epochs.png", fig, px_per_unit = 2)
end
