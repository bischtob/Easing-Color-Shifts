...
function img_plot(samples, savepath, plotname; ncolumns = 10,FT=Float32, logger=nothing)
    ...
    if !(logger isa Nothing)
        EasingColorShifts.log_artifact(logger, joinpath(savepath, plotname); name=plotname, type="PNG-file")
    end
end
...
function timewise_score_matching_loss(model, x_0; ϵ=1.0f-5, c=nothing)
    ...
    z = randn!(similar(x_0))
    μ_t, σ_t = EasingColorShifts.marginal_prob(model, x_0, t)
    x_t = @. μ_t + σ_t * z

    # evaluate model score s₀(𝘹(𝘵), 𝘵)
    s_t = EasingColorShifts.score(model, x_t, t; c=c)
...
"""
    setup_SDEProblem(model::EasingColorShifts.AbstractDiffusionModel, init_x, nsteps::Int; ϵ=1.0f-5, reverse::Bool=false, t_end=1.0f0)
...
"""
function setup_SDEProblem(model::EasingColorShifts.AbstractDiffusionModel, init_x, nsteps::Int; c=nothing,ϵ=1.0f-5, reverse::Bool=false, t_end=1.0f0)
    if reverse
        time_steps = LinRange(t_end, ϵ, nsteps)
        f,g = EasingColorShifts.reverse_sde(model)
        Δt = time_steps[1] - time_steps[2]
    else
        time_steps = LinRange(ϵ, t_end, nsteps)
        f,g = EasingColorShifts.forward_sde(model)
...
"""
    setup_ODEProblem(model::EasingColorShifts.AbstractDiffusionModel, init_x, nsteps::Int; ϵ=1.0f-5, reverse::Bool=false, t_end=1.0f0)
...
quantity by defintion.
"""
function setup_ODEProblem(model::EasingColorShifts.AbstractDiffusionModel, init_x, nsteps::Int; c=nothing,ϵ=1.0f-5, reverse::Bool=false, t_end=1.0f0)
    if reverse
        time_steps = LinRange(t_end, ϵ, nsteps)
        f= EasingColorShifts.probability_flow_ode(model)
        Δt = time_steps[1] - time_steps[2]
    else
        time_steps = LinRange(ϵ, t_end, nsteps)
        f= EasingColorShifts.probability_flow_ode(model)
...
"""
function model_gif(model::EasingColorShifts.AbstractDiffusionModel, init_x, nsteps::Int, savepath::String, plotname::String;
...
"""
    diffusion_simulation(model::EasingColorShifts.AbstractDiffusionModel,
                         init_x,
                         nsteps::Int;
                         c=nothing,
                         reverse::Bool=false,
...
"""
function diffusion_simulation(model::EasingColorShifts.AbstractDiffusionModel,
                              init_x,
                              nsteps::Int;
                              c=nothing,
                              reverse::Bool=false,
...
"""
    adapt_x!(x,
            forward_model::EasingColorShifts.VarianceExplodingSDE,
            reverse_model::EasingColorShifts.VarianceExplodingSDE,
            forward_t_end::FT,
...
function adapt_x!(x,
                 ...
                 forward_t_end::FT,
                 reverse_t_end::FT) where{FT}
    _, forward_σ_end = EasingColorShifts.marginal_prob(forward_model, x, FT(forward_t_end)) # x only affects the mean, which we dont use
    _, reverse_σ_end = EasingColorShifts.marginal_prob(reverse_model, x, FT(reverse_t_end)) # x only affects the mean, which we dont use
    @. x = x * reverse_σ_end / forward_σ_end
end

"""
    diffusion_bridge_simulation(forward_model::EasingColorShifts.VarianceExplodingSDE,
                                reverse_model::EasingColorShifts.VarianceExplodingSDE,
                                init_x,
                                nsteps::Int,
                                ;
...
bridge.
"""
function diffusion_bridge_simulation(forward_model::EasingColorShifts.VarianceExplodingSDE,
                                     reverse_model::EasingColorShifts.VarianceExplodingSDE,
                                     init_x,
                                     nsteps::Int,
                                     ;
