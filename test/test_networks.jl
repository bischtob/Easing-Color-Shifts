...
@testset "AttentionBlock" begin
 ...
 @testset "DDPM Network" begin
    net = EasingColorShifts.DenoisingDiffusionNetwork(inchannels=3)
    ps = Flux.params(net)
    k = 5
    ...
    if CUDA.has_cuda()
        net = EasingColorShifts.DenoisingDiffusionNetwork(inchannels=3) |> Flux.gpu
        ps = Flux.params(net)
        k = 5
...
@testset "NCSN" begin
    net = EasingColorShifts.NoiseConditionalScoreNetwork(;noised_channels=2, outer_kernelsize=5, channels=[32, 64, 128, 256])
    ...
    shift_input_net = EasingColorShifts.NoiseConditionalScoreNetwork(;noised_channels=2, shift_input=true, outer_kernelsize=5,)
    Flux.loadmodel!(shift_input_net, net)
    @test net(x.-mean(x, dims = (1,2)), c, t) ≈ shift_input_net(x, c, t)

    shift_output_net = EasingColorShifts.NoiseConditionalScoreNetwork(;noised_channels=2, shift_output=true, outer_kernelsize=5,)
    Flux.loadmodel!(shift_output_net, net)
    @test (net(x, c, t) .-mean(net(x,c, t), dims = (1,2))) ≈ shift_output_net(x, c, t)

    mean_bypass_net = EasingColorShifts.NoiseConditionalScoreNetwork(;noised_channels=2, shift_input=true, shift_output=true, mean_bypass=true)
    ps = Flux.params(net)
    k = 5
    x = rand(Float32, 2^k, 2^k, 2, 11)
    t = rand(Float32)
...
@testset "NCSN in and out channels" begin
    # with context
    net = EasingColorShifts.NoiseConditionalScoreNetwork(context=true, noised_channels=2, context_channels = 3)
    ps = Flux.params(net)
    k = 5
    x = rand(Float32, 2^k, 2^k, 2, 11)
    ...
    net2 = EasingColorShifts.NoiseConditionalScoreNetwork(noised_channels=3)
    ps = Flux.params(net2)
    k = 5
    x = rand(Float32, 2^k, 2^k, 3, 11)
    t = rand(Float32)
