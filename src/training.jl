...
function train!(model,
    ...
    for epoch = start_epoch:nepochs
        ...
        EasingColorShifts.update_step!(ps, ps_smooth, opt, opt_smooth, dataloaders.loader_train, lossfn, device)
        ...
        EasingColorShifts.save_model_and_optimizer(Flux.cpu(model), Flux.cpu(model_smooth), opt, opt_smooth, savepath)
...
"""
    EasingColorShifts.update_step!(ps, ps_smooth, opt, opt_smooth, loader_train, lossfn::Function, device::Function)
...
"""
    EasingColorShifts.compute_losses(lossfn, dataloaders, device::Function)
...
"""
    EasingColorShifts.save_model_and_optimizer(model, model_smooth, opt, opt_smooth, path::String)
...
"""
    EasingColorShifts.load_model_and_optimizer(path::String)
