"""
    Euler_Maruyama_sampler(model::EasingColorShifts.AbstractDiffusionModel,
                           init_x::A,
                           time_steps,
                           Δt;
                           c=nothing,
...
"""
function Euler_Maruyama_sampler(model::EasingColorShifts.AbstractDiffusionModel,
                                init_x::A,
                                time_steps,
                                Δt;
                                c=nothing,
    ...
    @showprogress "Euler-Maruyama Sampling" for time_step in time_steps
        ...
        g = EasingColorShifts.diffusion(model, batch_time_step)
        if forward
            x = x .+ sqrt(Δt) .* EasingColorShifts.expand_dims(g, 3) .* randn!(similar(x))
        else
        score = EasingColorShifts.score(model, x, batch_time_step; c=c)
        mean_x = x .+ EasingColorShifts.expand_dims(g, 3) .^ 2 .* score .* Δt
        x = mean_x .+ sqrt(Δt) .* EasingColorShifts.expand_dims(g, 3) .* randn!(similar(x))
        end
    end
    return x
end
...
"""
    predictor_corrector_sampler(model::EasingColorShifts.AbstractDiffusionModel,
                                init_x::A,
                                time_steps,
                                Δt;
                                snr=0.16f0,
...
"""
function predictor_corrector_sampler(model::EasingColorShifts.AbstractDiffusionModel,
                                     init_x::A,
                                     time_steps,
                                     Δt;
                                     snr=0.16f0,
    ...
    @showprogress "Predictor Corrector Sampling" for time_step in time_steps
        ...
        grad = EasingColorShifts.score(model, x, batch_time_step; c=c)
        ...
        mean_x = x .+ EasingColorShifts.expand_dims((g .^ 2), 3) .* grad .* Δt
        x = mean_x + sqrt.(EasingColorShifts.expand_dims((g .^ 2), 3) .* Δt) .* randn!(similar(x))
    end
    return x
end

"""
    setup_sampler(model::EasingColorShifts.AbstractDiffusionModel,
                  device,
                  tilesize,
                  noised_channels;
                  num_images = 5,
...
"""
function setup_sampler(model::EasingColorShifts.AbstractDiffusionModel,
                       device,
                       tilesize,
                       noised_channels;
                       num_images = 5,
    ...
    _, σ_T = EasingColorShifts.marginal_prob(model, zero(init_z), t)
    init_x = (σ_T .* init_z)
